...
| | |
|---|---|
| **状态** | **已验证**（2026-08-09）—— 见 [ADR 状态机](goals.md#07-adr-状态机v19)。[§5](#5-spike-方案) 六条判据由 [spike-002](spike-002-ecs.md) 全部跑通，**koota 锁定**。<br>**实测补充**：koota 的重放确定性成立，但**迭代顺序不是稳定 id 升序**（id 复用后错位）→ 触发 [§6](#6-退役条件按-goalsmd-72-预先写死) 升级路径 ①，**封装层必须排序**。这是 M1-a 的硬职责，不是优化 |
| **证据档** | **T3** · 可执行 oracle —— [spike-002](spike-002-ecs.md) 六条判据实跑。见 [§0.7 证据分档](goals.md#07-adr-状态机v19) |
| **语义类别** | **运行期语义** |
| **日期** | 2026-08-08 |
| **决策者** | 项目所有者 |
| **上游** | [goals.md](goals.md) **决策 5**（实体模型 = ECS）、**§7.1**（模块契约）、**§9**（未决项）、**决策 29**（确定性是硬要求） |

---

## 1. 背景

[goals.md](goals.md) **决策 5** 定了实体模型用 ECS。本 ADR 决定**用哪个实现**。

选型必须服务的，是 goals.md 里已经定死的几件事，而不是「哪个 ECS 更好」这种泛问：

> **v19 修正**：下表的节号与决策号原本全部指向旧版 goals.md 的坐标系，
> 十几版重构后**六处无一对得上**（例如「决策 4」当时是 ECS，现在是「只做 3D」）。
> 已按 v19 的编号重新对齐。**裸引用不是 markdown 链接，校验器查不出来**——
> 这是 v19 给 `check-docs.py` 加裸引用校验的直接原因。

| 约束来源 | 对 ECS 的要求 |
|---|---|
| **§4.5** 两级状态与播放沙箱 | 全世界快照 + 回滚 |
| **§4.2** Action 层 | 变更可追踪、可 diff、可回放 |
| **§7.3 + 决策 29** 试飞台与确定性 | **确定性**：同一 Action 序列 → 同一序列化结果，**硬要求** |
| **§7.1** 模块契约 | 组件类型可在运行时由模块注册 |
| **决策 6** JS 实现条件与动作 | 由**人和 AI**书写，API 出错率是一等指标 |
| **§5.3** 详情面板 | 组件 schema 可自省，用于自动生成控件 |
| 商业化可能性 | 许可证不能带传染性约束 |
...
脚本作者是**开发者和 AI**（**决策 6 + §4.3**）。AI 写第一种形式出错率显著更高，
...
2. **§7.1** 模块契约本来就禁止「直接读写不属于自己的组件表」，封装层是它的执行点
3. **选型从「地基决策」降级为「可替换模块」**，**§7.2** 的退役条件因此才可执行
...
自研的控制力，封装层已经给了；而 relations / prefabs / 归档查询这些白送的能力，自己补要更久。
**注**：若 spike 全部失败，自研仍是 [§6](#6-退役条件按-goalsmd-72-预先写死) 升级路径的第 ② 步，只是不作为首选。

---

## 5. Spike 方案

> **技术止损时间盒：3 天**（goals.md §0.5 第 1 条允许——它是实验的退出条件，不是交付承诺）。
...
## 6. 退役条件（按 goals.md §7.2 预先写死）

> **v19 重写。** 原文的最后一条是「超时则接受**仅编辑期确定性**，并把回放降级为
> 『Action 日志可读但不保证逐字节复现』」——**它与 goals.md 决策 29 正面冲突**。
> 决策 29（v14 确立，v17 被八款目标游戏证成）写明**逐帧确定性是硬要求、不允许降级**。
> 本 ADR 写于决策 29 之前，那条逃生路线是历史残留。
> **一份 ADR 不能给出与最高裁决依据相反的失败处理策略**——工程师会从两处读到两套做法。

**换库的退役条件**：

- spike 中任一库在**止损时间盒（3 天）**内跑不通确定性测试 → **淘汰该库**
- 封装层落地后，若日后需要换库 → **换**，代价已被封装层限制在约 800 行内

**确定性守不住时的升级路径**（**唯一路径，不允许静默降级**）：

```
库自身不能保证稳定的查询迭代顺序
  → ① 封装层统一按稳定实体 id 排序后再交给系统
  → ② 仍不满足则实现最小确定性 ECS（只做 goals.md §5.1「世界」那一节要求的部分）
  → ③ 仍不可行则停止实施，显式重审 goals.md 决策 29
  → 不允许「接受仅编辑期确定性」这类静默降级
```

第 ③ 步的含义是**把问题交回产品层**：要么接受重新定义联机路线（决策 29、阶段五一并重估），
要么接受更长的自研投入。**这是一次公开的决策，不是一次实现层的妥协。**

`①` 应该覆盖绝大多数情况——[§5](#5-spike-方案) 的第 5 条测的正是这个，
而排序在封装层做，两个候选库都不需要改。
...
- **构建与依赖管理**：当前无 `package.json`、无打包器，three 走 CDN importmap。
  引入 ECS + Rapier + glTF 后大概率绕不开构建步骤。**已提前到阶段零**（goals.md §6）
