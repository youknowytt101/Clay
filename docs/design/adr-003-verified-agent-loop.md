...
| | |
|---|---|
| **状态** | **倾向已定** —— 见 [ADR 状态机](goals.md#07-adr-状态机v19)。转「已验证」的条件：[§10](#10-三个手工样例与冻结条件) 三个手工样例逐条走通；再由最小纵切实现验证事务与错误通过率后转「已冻结」。**v23 补**：本文是**协议治理**类，进「已验证」需 T2（隔离影子回放，带植入负例与无问题对照），§10 三个手工样例本身是 T1，**走通样例不足以转档** |
| **证据档** | **T1** · 手工推演（§10 三个样例尚未走通）—— 见 [§0.7 证据分档](goals.md#07-adr-状态机v19) |
| **语义类别** | **协议治理** |
