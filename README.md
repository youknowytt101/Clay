...
处于**倾向已定**——三个手工样例经[两轮推演](docs/design/adr-002-walkthrough.md)走通，暴露并补上 6 处语义缺口；
但按 **v23 的证据分档**（决 43），推演是 T1 证据，运行期语义类要 T3 才能转「已验证」。
**求值器还没有一行实现**，转档的条件是那三个样例成为可执行测试。

> **v17 重定了方向**（当前 **v23**，见 goals.md 开头）：目标用户从「不写代码的普通人」改为**有经验的游戏研发人员**，
...
| | 产出 |
|---|---|
| ⏳ **求值语义定稿**（决策 31） | [ADR-002](docs/design/adr-002-eventsheet-eval.md) **r2 · 倾向已定**——[两轮推演](docs/design/adr-002-walkthrough.md)走通三个样例，补上 6 处语义缺口。**v23 由「已验证」退回**：推演是 T1，运行期语义要 T3（决 43），差可执行求值器 |
...
> ADR-001 为「已验证」（T3 · spike 实跑）；ADR-002 在 v23 退回「倾向已定」，
> 因为它的证据是 T1 推演而它是运行期语义类（决 43）。但它换成了一条更强的：
> **M1 期间新增的任何协议字段，都要有一个跑得起来的测试**，
> 而不是又一段纸面规范。ADR-001 的排序职责已经做到了这一点，
> **ADR-002 的 D4 还没有**——它正是 v23 退档要暴露的那件事。
